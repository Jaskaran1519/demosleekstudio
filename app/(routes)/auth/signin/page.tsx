...
function SignInForm() {
  ...
  const [rememberMe, setRememberMe] = useState(true); // Default to true as per design
  const [isLoading, setIsLoading] = useState(false);
  const [isGoogleLoading, setIsGoogleLoading] = useState(false);
  const [error, setError] = useState("");

  const pageTitle = "Sleek Studio"; // Can be "Welcome back, Olivia" if you have user's name
  const pageSubtitle = "Welcome back! Please enter your details.";


  useEffect(() => {
    if (errorFromUrl) {
      if (errorFromUrl === "OAuthAccountNotLinked") {
        setError("An account already exists with this email address using a different sign-in method. Please use the original method.");
  ...
  return (
    <div className="flex min-h-screen w-full bg-white"> {/* Main container for two panels */}
      {/* Left Panel (Form) */}
      <div className="flex flex-1 flex-col justify-center py-12 px-4 sm:px-6 lg:w-1/2 lg:flex-none lg:px-20 xl:px-28">
        <div className="mx-auto w-full max-w-sm lg:w-96">
          <div className="mb-10">
            ...
            <p className="mt-3 text-base text-gray-600">{pageSubtitle}</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <p className="mt-8 text-center text-sm text-gray-600">
              ...
              <Link href="/auth/signup" className="font-semibold text-indigo-600 hover:text-indigo-500 hover:underline">
                Sign up
              </Link>
            </p>
          </div>
      ...
      <div className="hidden lg:relative lg:block lg:w-1/2 bg-[#F9FAFB]"> {/* Light background for the image panel */}
        <Image
          className="absolute inset-0 h-full w-full object-contain p-12" // Use object-contain for logo, adjust padding

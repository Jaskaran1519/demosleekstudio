...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
    fetchProducts(1, true);
  }, [categoryParams.join(","), clothTypeParams.join(","), sortParam, searchParams.get("search")]);
  ...
  const fetchProducts = async (page: number, reset: boolean = false) => {
    try {
      ...
      console.log(`API response: ${data.products.length} products out of ${data.totalCount}`);
  ...
  const updateUrlWithFilters = (updatedParams: Record<string, string | null>) => {
    ...
    <Container className="py-5 md:px-8">
      <div className="flex flex-col space-y-6">
        ...
        <div>
          ...
          {/* Active filters */}
          {(categoryParams.length > 0 || clothTypeParams.length > 0 || debouncedSearchTerm || sortParam) && (
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {categoryParams.length > 0 && categoryParams.map((category) => (
                <Badge key={category} variant="secondary" className="px-3 py-1">
                  Category: {category.replace(/_/g, ' ')}
                </Badge>
              ))}
              
              {clothTypeParams.length > 0 && clothTypeParams.map((type) => (
              ...
              {sortParam && (
                <Badge variant="secondary" className="px-3 py-1">
                  Sort: {
                    ...
                    sortParam === "popularity" ? "Popularity" : "Newest"
                  }
                </Badge>
              )}
